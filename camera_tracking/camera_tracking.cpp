    ...
    while(1)
    {
        ...
        if(mode == CAPTURING || mode == DETECTION) {
            ...
            if(found) {
                ...
                objectPoints.resize(1, objectPoints[0]);
                ...
                for(int i = 0; i < pointbuf.size(); i ++) {
                    ...
                    // circle(view, textOrigin1, 10, Scalar(0,0,255));
                }
            }


            // CAPTURING: save coners
        ...
        /* after calibration is done */
             ...
             *  Red 160-179
            **************************/
            // Scalar lower_blue(160,50,50);
            // Scalar upper_blue(179,255,255);
            Scalar lower_blue(38,50,50);
            Scalar upper_blue(75,255,255);
            Mat mask;
            ...
            int obj_size = 10;
            erode(mask, mask, getStructuringElement(MORPH_ELLIPSE, Size(obj_size, obj_size)) );
            dilate(mask, mask, getStructuringElement(MORPH_ELLIPSE, Size(obj_size, obj_size)) ); 
            ...
            circle(view, ctr, 10, Scalar(0,255,255));

            double curt = (double (clock())) / CLOCKS_PER_SEC;
            printf("%f: img: (%.2f,%.2f), phy: (%.2f,%.2f)\n", curt, ctr.x, ctr.y, ctr_world.x, ctr_world.y);
            file_trace << curt << "\t" << ctr.x << "\t" << ctr.y << "\t" << ctr_world.x << "\t" << ctr_world.y << endl;
