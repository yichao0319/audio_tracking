...
function process_rx_chirp(filename, fc)
    ...
    subplot(211);
    plot(real(analogData(1:2*chirp_len)));
    xlabel('Time (s)'); ylabel('Amplitude (v)');
    title('FMCW signal'); axis tight;

    subplot(212);
    spectrogram(analogData(1:2*chirp_len),256,64,256,fs,'yaxis');
    set(gca, 'YLim', [0 5]);
    title('RX signal');
    % return
    ...
    % grid('on');
    % itvl_prev  = ((sync_idx(2:end) - sync_idx(1)) ./ (1:(Nc-1))) / gsize;
    % itvl_first = (sync_idx(2:end) - sync_idx(1:end-1)) / gsize;
    ...
    % new_chirp_len = 4407.965;
    new_chirp_len = 4407.964;
    fprintf('    select: %.2f\n', new_chirp_len);
    % return
